...
export const getAllUser = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      return res.status(404).json({ error: "pas de data" });
    }

    return res.status(201).json(user);
  } catch (error) {
    console.error("Erreur lors de la recuperation des data :", error);
...
export const loginUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const token = jwt.sign(
      ...
      JWT_SECRET_MOBILE,
      {
        expiresIn: "4h", // Durée de validité du token
      }
    );
...
export const imgUpload3 = async (req: Request, res: Response) => {
  const images = req.file?.filename;
  if (images) {
    return res
      .status(200)
